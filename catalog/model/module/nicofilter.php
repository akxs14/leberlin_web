...
class ModelModuleNicoFilter extends Model 
{
		...
		foreach ($filters as $filter_name => $filter_value)
		{
			if (strpos($filter_name, 'filter_') !== false)
			{
				...
				switch ($filter_name)
				{
					...
					$this->joins['price'] = ' LEFT JOIN ' . DB_PREFIX . 'product_special ps ON (p.product_id = ps.product_id) ';
					break;
					case 'price_max':
					if (is_array($ignore) && in_array('price', $ignore)) continue;
					$price = $this->substract_taxes($filter_value, true, $filters['category']);
					...
					break;
					case 'option':
					$this->joins['option'] = ' INNER JOIN ' . DB_PREFIX . 'product_option_value pov ON (p.product_id = pov.product_id) ';
					...
					*/
					...
					break;
					case 'attribute':
					$this->joins['attribute'] = ' INNER JOIN ' . DB_PREFIX . 'product_attribute pa  ON (p.product_id = pa.product_id) ';
	...
	{
		$cache_key = 'nico_filter.price.' . (int)$this->config->get('config_store_id') . '.' . $this->lang_code .'.' . md5($filter_sql);
	...
	{
		$cache_key = 'nico_filter.manufacturers.' . (int)$this->config->get('config_store_id') . '.' . $this->lang_code .'.' . md5($filter_sql);
	...
	{
		$cache_key = 'nico_filter.attributes.' . (int)$this->config->get('config_store_id') . '.' . $this->lang_code .'.' . md5($filter_sql);
		$data = $this->cache->get($cache_key);
	...
	{
		$cache_key = 'nico_filter.categories.' . (int)$this->config->get('config_store_id') . '.' . $this->lang_code .'.' . $parent_id;
		$data = $this->cache->get($cache_key);
	...
	{
		$cache_key = 'nico_filter.filters.' . (int)$this->config->get('config_store_id') . '.' . $this->lang_code .'.' . $category_id;
		$data = $this->cache->get($cache_key);
	...
	{
		$cache_key = 'nico_filter.options.' . (int)$this->config->get('config_store_id') . '.' . $this->lang_code .'.' . md5($filter_sql);
		$data = $this->cache->get($cache_key);
