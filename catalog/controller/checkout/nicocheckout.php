...
class ControllerCheckoutNicoCheckout extends Controller { 
	...
	{
		$data = array('checkout_hide_tax_id' => true,'checkout_hide_company_id' => true,'checkout_hide_company' => true, 'checkout_hide_fax' => true,'checkout_hide_address_2' => false,'checkout_hide_country' => false,'checkout_hide_zone' => false,'checkout_hide_city' => false,'checkout_hide_postcode' => false,'checkout_hide_gift' => false,'checkout_hide_coupon' => false,'checkout_hide_points' => false,'checkout_shipping_reload_payment' => false,'checkout_payment_auto_cofirm' => true);
		
		if ($config = $this->config->get('nicocheckout')) 
		{
			
			if (isset($config['nicocheckout_module'][1])) $config = $config['nicocheckout_module'][1];
			elseif (isset($config['nicocheckout_module'])) $config = $config['nicocheckout_module'];
			
		} elseif ($config = $this->config->get('nicocheckout_module')) 
		{
			if (isset($config[1])) $config = $config[1];
		}
		
		if ($config) $data = array_merge($data, $config);
		
		$opencart_version = $data['opencart_version'] = (int)str_replace('.','',VERSION);
		...
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			if ($data['opencart_version'] > 2000) $this->response->redirect($this->url->link('checkout/cart')); else $this->redirect($this->url->link('checkout/cart'));
		}
		...
		if (isset($this->session->data['customer_id']))
		{
			...
			if (isset($this->session->data['nicocheckout_customer_id']) && $this->session->data['nicocheckout_customer_id'] === true) 
			{
				...
				//unset($this->session->data['nicocheckout_customer_id']);
			}
			else 
			{
		...
		{
			$this->template  = $template = 'checkout/nicocheckout';
		} else
		...
		} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/nicocheckout.tpl')) {
			$this->template  = $template = 'default/template/checkout/nicocheckout.tpl';
		} else {
			$this->template  = $template = 'checkout/nicocheckout.tpl';
		}
	...
	public function login_validate($data = array()) {
		$opencart_version = $data['opencart_version'] = (int)str_replace('.','',VERSION);;
		if ($data['opencart_version'] > 2000) $this->load->language('checkout/checkout'); else $this->language->load('checkout/checkout');
	...
	public function guest_validate() {
		...
		if ($data['opencart_version'] > 2000)
		{
			...
			if (!$json) {
				...
				if ($country_info && isset($country_info['postcode_required']) && $country_info['postcode_required'] && (utf8_strlen(trim($this->request->post['postcode'])) < 2 || utf8_strlen(trim($this->request->post['postcode'])) > 10)) {
				...
				if (isset($this->request->post['customer_group_id']) && is_array($this->config->get('config_customer_group_display')) && in_array($this->request->post['customer_group_id'], $this->config->get('config_customer_group_display'))) {
					...
					if (!$customer_group_id) $customer_group_id = 1;
				}
			...
			if (!$json && $this->request->post) {
				...
				if (isset($this->session->data['guest']['shipping_address']) || isset($this->session->data['shipping_address'])) {
	...
	//			unset($this->session->data['payment_methods']);
		...
		} else 
		{
			...
			if (!$json) {
				...
				if ($country_info) {
					if (isset($country_info['postcode_required']) && $country_info['postcode_required'] && (utf8_strlen($this->request->post['postcode']) < 2) || (utf8_strlen($this->request->post['postcode']) > 10)) {
						$json['error']['postcode'] = $this->language->get('error_postcode');
					}
			...
			if (!$json && $this->request->post) {
				...
				$this->session->data['guest']['payment']['country_id'] = isset($this->request->post['country_id'])?$this->request->post['country_id']:'';
				$this->session->data['guest']['payment']['zone_id'] = isset($this->request->post['zone_id'])?$this->request->post['zone_id']:'';
	...
	public function register_validate(&$data = array()) 
	{
		...
		if (!$json) {	
			...
			if ($data['opencart_version'] > 2000) $this->load->model('account/customer');	
			...
			if ($country_info) {
				if (isset($country_info['postcode_required']) && $country_info['postcode_required'] && (!isset($this->request->post['postcode']) || (utf8_strlen($this->request->post['postcode']) < 2) || (utf8_strlen($this->request->post['postcode']) > 10))) {
					$json['error']['postcode'] = $this->language->get('error_postcode');
				}
		...
		if (!$json) {
			...
			if ($data['opencart_version'] > 2000)
			{
				if (!$this->customer->isLogged())
				{
					...
					$this->session->data['nicocheckout_customer_id'] = $customer_id = $this->model_account_customer->addCustomer($this->request->post);
	...
	if ($data['opencart_version'] > 2000) 
		{
				if (!$json) 
				{
					if (isset($this->request->post['payment_address']) && $this->request->post['payment_address'] == 'existing') {
						...
						if (isset($country_info) && isset($country_info['postcode_required']) && $country_info['postcode_required'] && (utf8_strlen(trim($this->request->post['postcode'])) < 2 || utf8_strlen(trim($this->request->post['postcode'])) > 10)) {
							$json['error']['postcode'] = $this->language->get('error_postcode');
						}
	...
	public function shipping_address_validate(&$data = array()) {
		...
		if (!$json) {
			...
			if (isset($this->request->post['shipping_address']) && $this->request->post['shipping_address'] == 'new') { 
				...
				if ($country_info && isset($country_info['postcode_required']) && $country_info['postcode_required'] && (utf8_strlen($this->request->post['shipping_postcode']) < 2) || (utf8_strlen($this->request->post['shipping_postcode']) > 10)) {
				...
				if (!$json) {						
					...
					if (isset($this->request->post['shipping_country_id'])) $this->session->data['shipping_country_id'] = $this->request->post['shipping_country_id'];
					if (isset($this->request->post['shipping_zone_id'])) $this->session->data['shipping_zone_id'] = $this->request->post['shipping_zone_id'];
...
//				error_log(print_r($this->session->data['payment_methods'],1));
	...
	public function shipping_address($render = true, &$data  = array()) {
		...
		if ($render !== false)
		{
			...
			{
				$this->template  = $template = 'checkout/shipping_method';
			} else
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/shipping_method.tpl')) {
				$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/shipping_method.tpl';
			} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/shipping_method.tpl')) {
				$this->template  = $template = 'default/template/checkout/shipping_method.tpl';
			} else {
				$this->template  = $template = 'checkout/shipping_method.tpl';
			}
					
			$opencart_version = $data['opencart_version'] = (int)str_replace('.','',VERSION);;
			if (!$opencart_version > 2000) $this->data = $data;
			$this->response->setOutput($this->render());
		}
  	}		
	...
	public function shipping_method($render = true, &$data = array()) 
	{
		...
		$shipping_address = array('country_id' => 0, 'zone_id' => 0, 'city' => '', 'postcode' => '0','firstname' => '', 'lastname' => '', 'company' => '', 'address_1' => '');
/*
		...
		$postcode = $this->config->get('config_postcode')?$this->config->get('config_postcode'):0;
		...
		if (isset($this->request->post['shipping_address']))
		{
			if ($this->request->post['shipping_address'] == 'new')
			{
				...
				if (isset($this->request->post['zone_id']) && $this->request->post['zone_id']) $zone_id = $this->request->post['shipping_zone_id'];
				if (isset($this->request->post['city']) && $this->request->post['city']) $city = $this->request->post['shipping_city'];
				if (isset($this->request->post['postcode']) && $this->request->post['postcode']) $postcode = $this->request->post['shipping_postcode'];
			}
		} else if (isset($this->request->post['payment_address']) && $this->request->post['payment_address'] == 'new')
		{
			$country_id = '';
			if (isset($this->request->post['country_id']) && $this->request->post['country_id']) $country_id = $this->request->post['country_id'];
			if (isset($this->request->post['zone_id']) && $this->request->post['zone_id']) $zone_id = $this->request->post['zone_id'];
			if (isset($this->request->post['city']) && $this->request->post['city']) $city = $this->request->post['city'];
			if (isset($this->request->post['postcode']) && $this->request->post['postcode']) $postcode = $this->request->post['postcode'];
		}
		...
		if ($this->customer->isLogged())
		{
			...
			$shipping_address_id = $data['shipping_address_id'] = (isset($this->session->data['shipping_address_id'])?$this->session->data['shipping_address_id']:'');
			...
			} else if (isset($payment_address_id)) 
			{					
		...
		if (empty($shipping_address['postcode'])) $shipping_address['postcode'] = 0;
		if (empty($shipping_address['city'])) $shipping_address['city'] = ' ';
		if (empty($shipping_address['iso_code_2'])) $shipping_address['iso_code_2'] = ' ';
		if (empty($shipping_address['iso_code_3'])) $shipping_address['iso_code_3'] = ' ';
		...
		if ($render !== false)	
		{
			...
			if ($opencart_version >= 2200) 
			{
				$this->template  = $template = 'checkout/nico_shipping_method';
			} else
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/nico_shipping_method.tpl')) {
				$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/nico_shipping_method.tpl';
			} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/nico_shipping_method.tpl')) {
				$this->template  = $template = 'default/template/checkout/nico_shipping_method.tpl';
			} else {
				$this->template  = $template = '/template/checkout/nico_shipping_method.tpl';
			}
	...
	public function payment_address($render = true, &$data  = array()) {
		...
		if ($render !== false)	
		{
			if ($opencart_version >= 2200) 
			{
				$this->template  = $template = 'checkout/payment_address';
			} else
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/payment_address.tpl')) {
				$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/payment_address.tpl';
			} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/payment_address.tpl')) {
				$this->template  = $template = 'default/template/checkout/payment_address.tpl';
			} else {
  	...
  	public function payment_method($render = true, &$data = array()) {
		...
		if ($render !== false)
		{
			...
			if ($opencart_version >= 2200) 
			{
				$this->template  = $template = 'checkout/nico_payment_method';
			} else
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/nico_payment_method.tpl')) {
				$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/nico_payment_method.tpl';
			} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/nico_payment_method.tpl')) {
				$this->template  = $template = 'default/template/checkout/nico_payment_method.tpl';
			} else {
				$this->template  = $template = 'checkout/nico_payment_method.tpl';
			}
	...
	public function checkout($render = true, &$data  = array()) 
	{
		...
		if ($opencart_version >= 2200) 
		{
			$this->template  = $template = 'checkout/checkout';
		} else
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/checkout.tpl')) {
			$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/checkout.tpl';
		} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/checkout.tpl')) {
  	...
  	public function guest($render = false, &$data  = array()) {
		...
		if ($render !== false)
		{
			if ($opencart_version >= 2200) 
			{
				$this->template  = $template = 'checkout/guest';
			} else
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/guest.tpl')) {
				$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/guest.tpl';
			} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/guest.tpl')) {
				$this->template  = $template = 'default/template/checkout/guest.tpl';
			} else {
	...
	public function login($render = false, &$data  = array()) {
		...
		if ($render !== false)
		{
			if ($opencart_version >= 2200) 
			{
				$this->template  = $template = 'checkout/login';
			} else
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/login.tpl')) {
				$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/login.tpl';
			} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/login.tpl')) {
				$this->template  = $template = 'default/template/checkout/login.tpl';
			} else {
	...
	public function cart($render = true, &$data = array()) 
	{
		...
		if ($data['opencart_version'] > 2000) $this->load->language('checkout/cart'); else $this->language->load('checkout/cart');
		...
		if ($opencart_version >= 2200)
		{
			if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
				...
				if ($opencart_version >= 2200) 
				{
					$this->template  = $template = 'checkout/nico_cart';
				} else
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/nico_cart.tpl')) {
					$this->template  = $template =  $this->config->get('config_template') . '/template/checkout/nico_cart.tpl';
				} else if (file_exists(DIR_TEMPLATE . '/default/template/checkout/nico_cart.tpl')) {
					$this->template  = $template = 'default/template/checkout/nico_cart.tpl';
				} else {
					$this->template  = $template = 'checkout/nico_cart.tpl';
				}
		...
		} else
		{
            ...
            foreach ($products as $product) {
		     ...
		     'cart_id'    => isset($product['cart_id'])?$product['cart_id']:'',
                    ...
                    'total'               => $total,
                    'href'                => $this->url->link('product/product', 'product_id=' . $product['product_id']),
                    'remove'              => $this->url->link('checkout/cart', 'remove=' . (isset($product['cart_id'])?$product['cart_id']:$product['key'])),
	...
	//										if ($result['code'] == 'shipping')
				...
				foreach ($total_data as $key => &$value) {
					if (!isset($value['text'])) $value['text']  = $this->currency->format($value['value'], $this->session->data['currency']);
					$sort_order[$key] = $value['sort_order'];
				}
	...
	public function confirm($render = true, &$data = array()) {
		...
		if ($data['opencart_version'] > 2000) {
			...
			$data['voucher_title'] = $this->language->get('text_use_voucher');
			$data['coupon_title'] = $this->language->get('text_use_coupon');
			$data['reward_title'] = sprintf($this->language->get('text_use_reward'), $points);
...
/*
				if (!isset($this->session->data['shipping_address'])) {
					$redirect = $this->url->link('checkout/nicocheckout', '', true);
				}
...
*/
			...
			foreach ($products as $product) {
				...
				if (isset($this->session->data['payment_address']) && is_array($this->session->data['payment_address']) && isset($this->session->data['payment_address']['firstname']))
				{
				...
				if ($order_data['currency_value'] == 0) $order_data['currency_value'] = $this->currency->getValue($this->currency->getCode());
...
*/
			...
			if (isset($_POST) && !empty($_POST))
			{
				...
				if ($this->cart->hasShipping()) {
					if (isset($this->request->post['shipping_firstname']) && !empty($this->request->post['shipping_firstname']))
					{
						...
						if (isset($this->request->post['shipping_country_id'])) $order_data['shipping_country_id'] = $this->request->post['shipping_country_id'];
						...
						if (isset($this->request->post['shipping_zone_id']) && $this->request->post['shipping_zone_id'])
						{
							...
							if ($zone_info)	$order_data['shipping_zone'] = $zone_info['name'];;//$this->model_localisation_zone->getZone($this->request->post['shipping_zone_id']);
						}						
					} 
					...
					{
						$order_data['shipping_firstname'] = isset($this->request->post['firstname'])?$this->request->post['firstname']:isset($order_data['payment_firstname'])?$order_data['payment_firstname']:'';
						$order_data['shipping_lastname'] = isset($this->request->post['lastname'])?$this->request->post['lastname']:isset($order_data['payment_lastname'])?$order_data['payment_lastname']:'';
						$order_data['shipping_company'] = isset($this->request->post['company'])?$this->request->post['company']:isset($order_data['payment_company'])?$order_data['payment_company']:'';
						$order_data['shipping_address_1'] = isset($this->request->post['address_1'])?$this->request->post['address_1']:isset($order_data['payment_address_1'])?$order_data['payment_address_1']:'';
						$order_data['shipping_address_2'] = isset($this->request->post['address_2'])?$this->request->post['address_2']:isset($order_data['payment_address_2'])?$order_data['payment_address_2']:'';
						$order_data['shipping_city'] = isset($this->request->post['city'])?$this->request->post['city']:isset($order_data['payment_city'])?$order_data['payment_city']:'';
						$order_data['shipping_postcode'] = isset($this->request->post['postcode'])?$this->request->post['postcode']:isset($order_data['payment_postcode'])?$order_data['payment_postcode']:'';
						$order_data['shipping_zone'] = isset($this->request->post['zone'])?$this->request->post['zone']:isset($order_data['payment_zone'])?$order_data['payment_zone']:'';
						$order_data['shipping_zone_id'] = isset($this->request->post['zone_id'])?$this->request->post['zone_id']:isset($order_data['payment_zone_id'])?$order_data['payment_zone_id']:'';
						$order_data['shipping_country'] = isset($this->request->post['country'])?$this->request->post['country']:isset($order_data['payment_country'])?$order_data['payment_country']:'';
						$order_data['shipping_country_id'] = isset($this->request->post['country_id'])?$this->request->post['country_id']:isset($order_data['payment_country_id'])?$order_data['payment_country_id']:'';
						$order_data['shipping_address_format'] = isset($this->request->post['address_format'])?$this->request->post['address_format']:isset($order_data['payment_address_format'])?$order_data['payment_address_format']:'';
						$order_data['shipping_custom_field'] = array();
						$order_data['shipping_method'] = isset($this->request->post['method'])?$this->request->post['method']:isset($order_data['payment_method'])?$order_data['payment_method']:'';
						$order_data['shipping_code'] = isset($this->request->post['code'])?$this->request->post['code']:isset($order_data['payment_code'])?$order_data['payment_code']:'';
					}
			...
			foreach ($products as $product) {
				...
				if (isset($_POST) && !empty($_POST))
				{
					if ($this->customer->isLogged()) {
						...
						if (!$order_data['customer_group_id']) $order_data['customer_group_id'] = 1;
					...
					if ((isset($payment_address) && is_array($payment_address)) || isset($this->session->data['payment_address']))
					{
							if (empty($payment_address) && isset($this->session->data['payment_address'])) $payment_address = $this->session->data['payment_address'];
					...
					if ($this->cart->hasShipping()) {
						...
						if ($this->customer->isLogged() && (isset($this->request->post['shipping_address_id']) || isset($this->session->data['shipping_address_id']))) 
						{
							$this->load->model('account/address');
	...
	public function voucher() 
	{
		...
		if ($opencart_version > 2000) {
			...
			$this->language->load('checkout/cart');
			$this->load->model('checkout/voucher');
			$error_voucher = $error_empty = $this->language->get('error_voucher');
			$text_success = $this->language->get('text_voucher');
			$voucher_info = $this->model_checkout_voucher->getVoucher($voucher);
		}
		...
		if (empty($this->request->post['voucher'])) {
			...
			//$json['redirect'] = $this->url->link('checkout/nicocheckout');
		} else {
	...
	public function coupon() 
	{
		...
		if ($opencart_version > 2000) {
			...
			$this->language->load('checkout/cart');
			$this->load->model('checkout/coupon');
			$error_coupon = $error_empty = $this->language->get('error_coupon');
			$text_success = $this->language->get('text_coupon');
			$coupon_info = $this->model_checkout_coupon->getCoupon($coupon);
		}
	...
	public function reward() 
	{
		...
		if ($opencart_version > 2000) {
			...
			$this->language->load('checkout/cart');
			$error_reward = $error_empty = $this->language->get('error_reward');
			$error_points = $error_empty = $this->language->get('error_points');
			$error_maximum = $error_empty = $this->language->get('error_maximum');
			$text_success = $this->language->get('text_reward');
		}
