...
class ControllerModuleNicoSocialAuth extends NicoModule
{
    ...
    public function index() {
        ...
        if ($this->customer->isLogged()) {
            if ($opencart2) $this->response->redirect($this->_redirect); else $this->redirect($this->_redirect);
        }


        // Dependencies
        $this->language->load('module/nicospeedcache');
        require_once(DIR_SYSTEM . 'library/Hybrid/Auth.php');
        $this->load->model('module/nicospeedcache');

        // Load Config
        $this->_config['base_url']   = $this->url->link('module/nicospeedcache/process');
        $this->_config['debug_file'] = DIR_SYSTEM . 'logs/hybridauth.txt';
        $this->_config['debug_mode'] = (bool) $this->config->get('hybrid_auth_debug');

        $settings = $this->config->get('nicospeedcache');
        ...
        try {
            ...
            $customer_id = $this->model_module_nicospeedcache->findCustomerByIdentifier($provider, $user_profile->identifier);

            if ($customer_id) {
                // 1.1 Login
                $this->model_module_nicospeedcache->login($customer_id);
            ...
            if ($user_profile->email){
                $customer_id = $this->model_module_nicospeedcache->findCustomerByEmail($user_profile->email);
            ...
            $customer_id = $this->model_module_nicospeedcache->addCustomer(
                array('email'      => $user_profile->email,
                      ...
                      'country_id' => $this->model_module_nicospeedcache->findCountry($user_profile->country),
                      'zone_id'    => $this->model_module_nicospeedcache->findZone($user_profile->region),
                      'password'   => substr(rand().microtime(), 0, 6)));

            // 3.2 - create a new authentication for him
            $this->model_module_nicospeedcache->addAuthentication(
