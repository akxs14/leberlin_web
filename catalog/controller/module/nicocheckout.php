...
class ControllerModuleNicoSocialAuth extends NicoModule
{
    ...
    public function index() {
        ...
        // Dependencies
        $this->language->load('module/nicocheckout');
        require_once(DIR_SYSTEM . 'library/Hybrid/Auth.php');
        $this->load->model('module/nicocheckout');

        // Load Config
        $this->_config['base_url']   = $this->url->link('module/nicocheckout/process');
        $this->_config['debug_file'] = DIR_SYSTEM . 'logs/hybridauth.txt';
        $this->_config['debug_mode'] = (bool) $this->config->get('hybrid_auth_debug');

        $settings = $this->config->get('nicocheckout');
        ...
        try {
            ...
            $customer_id = $this->model_module_nicocheckout->findCustomerByIdentifier($provider, $user_profile->identifier);

            if ($customer_id) {
                // 1.1 Login
                $this->model_module_nicocheckout->login($customer_id);
            ...
            if ($user_profile->email){
                $customer_id = $this->model_module_nicocheckout->findCustomerByEmail($user_profile->email);
            ...
            $customer_id = $this->model_module_nicocheckout->addCustomer(
                array('email'      => $user_profile->email,
                      ...
                      'country_id' => $this->model_module_nicocheckout->findCountry($user_profile->country),
                      'zone_id'    => $this->model_module_nicocheckout->findZone($user_profile->region),
                      'password'   => substr(rand().microtime(), 0, 6)));

            // 3.2 - create a new authentication for him
            $this->model_module_nicocheckout->addAuthentication(
