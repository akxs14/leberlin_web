...
function init_effects()
{
   ...
   _nico_changes['effect'] = {};
	effects();

    $("input#effects_random").on("change", function () {
			_nico_changes['effect']['random'] = jQuery("#effects_random").prop('checked');
            effects();
        });

    $("input#effects_duration").on("change", function () {
            jQuery(".effects_duration > span").html(this.value);
			_nico_changes['effect']['duration'] = jQuery("#effects_duration").val();
     });

    $("input#effects_delay").on("change", function () {
            jQuery(".effects_delay > span").html(this.value);
			_nico_changes['effect']['delay'] = jQuery("#effects_delay").val();
	 });

    $("select#effects_name").on("change", function () {
	   _nico_changes['effect']['name'] = jQuery("#effects_name").val();
        effects()
    });
}
...
function init_editor()
{
	...
	jQuery("#nico_save_settings").on("click", function(e) 
		...
		$(".nico_tabs li").click(function(e) 
		{
			...
			if (_effect_first_load && this.innerHTML == "Gallery effects")
			{
				init_effects();
				_effect_first_load = false;
			}
...
jQuery(document).ready(function()
{
		...
		$("#show_hide_editor").on("click", function (e) 
		{
			...
			{
				jQuery(this).css({marginTop:"-30px", paddingBottom:"50px", opacity:"0.2",'z-index':'-2', 'margin-left':'0px',display:"none"}).text("Hide panel");
			...
			{
				jQuery(this).css({right:"10px",marginTop:"-50px",paddingBottom:"50px", opacity:"1",'z-index':'-1', 'margin-left':'10px', display:"none"}).text("Customize theme");
...
function effects() {
	...
	var name = $("select#effects_name option:selected").val(),
		duration = parseInt($("input#effects_duration").val()),
		delay = parseInt($("input#effects_delay").val()),
		newDelay = 0,
		random = $("input#effects_random");

	/*if (random.is(":checked")) {
		$('#container .product').shuffleElements().each(function (i) {
			newDelay = i * delay;
		...
		$("#container .product").each(function (i) {
			newDelay = i * delay;
			$(this).attr("style", "-webkit-animation-delay:" + newDelay + "ms; "
				+ "-moz-animation-delay:" + newDelay + "ms; "
				+ "-o-animation-delay:" + newDelay + "ms; "
	...
	$("#container").attr("class","container-fluid effect effect_" + name);
//	masonryResize();
}
