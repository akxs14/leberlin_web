...
function addToCart(product_id, quantity) {
    ...
    $.ajax({
	    ...
	    success: function(json) {
		    ...
		    if (json['success']) {
			    $('#notification').html('<div class="information" style="display: none;"><span class="fa fa-check"></span><a class="close" href="#close"></a> ' + json['success'] + '</div>');
...
//		console.log(jQuery(this).parents("#product"));
		...
		$.ajax({
			...
			success: function(json) {
				...
				if (json['success']) {
					...
					//$('.breadcrumb').after('<div class="alert alert-success">' + json['success'] + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
					$('#notification').html('<div class="information" style="display: none;"><span class="fa fa-check"></span>' + json['success'] + '</div>');
					$('.information').fadeIn('slow');
	...
	jQuery("span.view").click(function (event) 
	{
...
$(document).on('ready', function () 
{
	...
	{ 
		...
		if (state.url != last_url && state.url.indexOf('route=product/product') > 0)
		{
			grid3D._hideContent();
			grid3D._showContent( 1, url);
	...
	{
		_grid3D._hideContent();
		if (document.location.href.indexOf('route=product/product') > 0)
		{
			_grid3D._showContent( 1, document.location.href);
		}
	};
...
jQuery(document).ready(function () 
{
	...
	var items = $("#container .product");
	if (rand) $("#container .product").shuffleElements();
	//<?php if (isset($_nico_settings['effect']['random']) && $_nico_settings['effect']['random'] == 'true') echo '.shuffleElements()'?>;
	var count = items.length;
	var time = animation_delay * (count);

	$("#container").addClass("effect effect_" + effect);

	items.each(function (i) {
		$(this).attr("style", "-webkit-animation-delay:" + i * animation_delay + "ms;"
				+ "-moz-animation-delay:" + i * animation_delay + "ms;"
				+ "-o-animation-delay:" + i * animation_delay + "ms;"
