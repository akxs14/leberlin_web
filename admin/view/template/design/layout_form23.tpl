        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-layout" class="form-horizontal">
          ...
          <fieldset>
            ...
            <div class="row">
			  ...
			  <?php 
              $theme_config = glob(dirname($_SERVER['SCRIPT_FILENAME']) . '/../catalog/view/theme/*/nico_theme_editor/theme_config.inc');
              $theme_config_file = $theme_config[0];
              include($theme_config_file);
              if (isset($nico_theme_positions)) {?>
              
              
              <legend><strong><?php echo ucfirst($nico_theme);?></strong> extra positions</legend>
              <div class="row">
              
              <?php
			  foreach ($nico_theme_positions as $value => $name) {
				
				//skip opencart default positions
				if (in_array($value, array('content_top', 'content_bottom', 'column_left', 'column_right'))) continue;
			  ?>
				  
              <div class="col-lg-3 col-md-4 col-sm-12">
				<table id="module-<?php echo $value;?>" class="table table-striped table-bordered table-hover">
                  <thead>
                    <tr>
                  ...
                  <tbody>
                    <?php foreach ($layout_modules as $layout_module) { ?>
                    <?php if ($layout_module['position'] == $value) { ?>
                    <tr id="module-row<?php echo $module_row; ?>">
                      <td class="text-left"><div class="input-group">
                          <select name="layout_module[<?php echo $module_row; ?>][code]" class="form-control input-sm">
                            <?php foreach ($extensions as $extension) { ?>
                  ...
                  <tfoot>
                    <tr>
                      <td class="text-left"><div class="input-group">
                          ...
                          <div class="input-group-btn">
                            <button type="button" onclick="addModule('<?php echo $value;?>');" data-toggle="tooltip" title="<?php echo $button_module_add; ?>" class="btn btn-primary btn-sm"><i class="fa fa-plus-circle"></i></button>
                          </div>
                        </div></td>
                    </tr>
                  </tfoot>
