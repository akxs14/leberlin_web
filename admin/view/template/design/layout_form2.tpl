<?php
global $nico_theme_name, $nico_theme_positions, $theme_name, $db, $_config, $nico_config, $load, $registry, $opencart2;
$db = $registry->get('db');;
if (!$db)
{
          ...
          <table id="module" class="table table-striped table-bordered table-hover">
            ...
            <tbody>
              ...
              <tr id="module-row<?php echo $module_row; ?>">
                ...
                <?php if (isset($extensions)) foreach ($extensions as $extension) { ?>
                ...
                <td class="text-left"><select name="layout_module[<?php echo $module_row; ?>][position]" class="form-control">
                	...
                	if ($config)
                	{
						if ($config->get('config_theme') == 'theme_default') {
							$directory = $config->get('theme_default_directory');
						} else {
							$directory = $config->get('config_theme');
						}

						if (empty($directory))
						{
							$directory = $config->get('config_template');
						}
					}
					$theme_config_file = dirname($_SERVER['SCRIPT_FILENAME']) . '/../catalog/view/theme/' . $directory . '/nico_theme_editor/theme_config.inc';
					include($theme_config_file);

					if (isset($nico_theme_positions)) {
                	foreach ($nico_theme_positions as $value => $name) {?>
                    <option value="<?php echo $value; ?>" <?php if ($layout_module['position'] == $value) { ?>selected="selected"<?php }?>><?php echo $name; ?></option>
                    <?php } } else {?>
...
function addModule() {
	...
	html += '    </optgroup>';
	<?php } ?>
	<?php } else foreach ($modules as $module) { ?>
