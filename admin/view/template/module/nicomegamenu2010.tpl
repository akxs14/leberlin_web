<?php 
global $module_name,  $autocomplete_products, $module_row, $module_name, $module_config, $config_size, $section_config, $section_text, $font_awesome_icons, $nico_theme_positions, $opencart_version, $categs, $_languages, $opencart_version;
$_languages = $languages;
if (isset($data['categories'])) $categs = $data['categories'];
...
$module_config = array(
	...
	'section' => array('custom', 'megamenu2010', 'Mega menu'),
);


function megamenu2010($module)
{
	global $_languages, $module_row, $categs, $_module_row, $opencart_version;
	...
	<div class="nico_menu">
			...
			{
				global $_languages,$lang_img_path, $module_name, $module_row, $_module_row, $categs, $opencart_version;
				var_dump($lang_img_path);
			?>	 
		    <ul>
 			<?php 
 			$i = 0;
			...
			<li <?php if (isset($category['categories'])){?> class="categories"<?php }?>>
				<a href="">
					...
					<?php $first=true;foreach ($_languages as $language) { ?>
					...
					<select _name="menu_type" class="menu_type" style="margin:10px 0px;">
					    ...
					    <option value="html" <?php if (isset($category['menu_type']) && $category['menu_type'] =='html') {?>selected=true<?php } ?>>Html</option>
					</select>
					...
					<?php if ($opencart_version < 2000) {?>
						<input type="text" _name="product" value="" autocomplete="off"/>
						<div class="scrollbox">
								<?php $class = 'odd'; ?>
								...
								<div class="product<?php echo $id; ?> <?php echo $class; ?>">
					...
					<?php } else {?>
						...
						<div id="autocomplete" class="well well-sm" style="height: 150px; overflow: auto;">
						  <?php if(isset($category['autocomplete'])) 
						  {
							  $products = array_combine(explode(',', $category['autocomplete']), explode(',', $category['autocomplete_names']));
							  foreach ($products as $id => $product) { ?>
						  <div id="autocomplete-<?php echo $id;?>"><i class="fa fa-minus-circle"></i> <?php  echo urldecode($product); ?>
							<input type="hidden" value="<?php echo $id; ?>" _label="<?php echo $product; ?>"/>
						  </div>
						  <?php } } ?>
						</div>
					...
					<div class="menu_section html"  <?php if ( isset($category['menu_type']) && $category['menu_type'] !='html') {?>style="display:none"<?php }?>>
					    <br/>
					    <?php $first=true;foreach ($_languages as $language) { ?>
					    <img src="<?php if ($opencart_version >= 2200) $image = $lang_img_path . $language['code']  .'/' . $language['code'] . '.png'; else  $image = $lang_img_path . $language['image'];echo $image;?>" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?> &nbsp; 
...
<div class="nico_new_menu_item_template" style="display:none">
     <div>
		...
		<?php if ($opencart_version < 2000) {?>
				<input type="text" _name="product" value="">
				<div id="product-autocomplete" class="scrollbox">
	    ...
	    <div class="menu_section html"  style="display:none">
		<?php $first=true;foreach ($_languages as $language) { ?>
		<img src="<?php if ($opencart_version >= 2200) $image = $lang_img_path . $language['code']  .'/' . $language['code'] . '.png'; else  $image = $lang_img_path . $language['image'];echo $image;?>" title="<?php echo $language['name']; ?>" /> <?php echo $language['name']; ?> &nbsp; 
		<textarea cols=80 rows=10 class="html_editor" _code="<?php echo $language['code'];?>"  _name="html" _name="html" id="text_<?php echo rand();?>"><?php if (isset($category['html'][$language['code']])) echo $category['html'][$language['code']]; ?></textarea>
		<?php } ?>
...
<?php if ($opencart_version < 2000) {?>
  CKEDITOR.config.height = 200;    
    ...
    if (jQuery('.menu_type',this).val() == 'html')
...
$treeview.jstree({ "core" : { "check_callback" : true }, "plugins" : [ "dnd", "state" ]})
...
jQuery(document.body).on("click", "a.nico_add_menu_item",function(e) 
{
	...
	{
	    ref = $(".nico_menu", this.parentNode).jstree({ "core" : { "check_callback" : true }, "plugins" : [ "dnd", "state" ]}).jstree(true);
	}
...
//	$.jstree.core.open_all($(".nico_menu"));
	...
	$(".nico_menu").each(function() 
	{
	    ...
	    jQuery('[name="menu"]').val(JSON.stringify(mega_menu));
	});
